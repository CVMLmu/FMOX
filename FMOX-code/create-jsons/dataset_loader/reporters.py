...
class GroundTruthProcessor:
	...
	def get_img_noupd(self, kk):
		...
		I = imread(path)
		original_I = cv2.imread(path)
		return I, original_I

	def get_img(self, kk):
		I, original_I = self.get_img_noupd(kk)
		B = np.median(self.Vk, 3)
		if kk >= self.mednused:
			self.Vk[:,:,:,:-1] = self.Vk[:,:,:,1:]
			self.Vk[:,:,:,-1] = I
		current_image_name = "{:08d}.png".format(self.start_ind+kk)
		return I,B, original_I, current_image_name
